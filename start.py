...
class Start:
    ...
    def buildFpm(self):
        ...
        fpm_container = self.d.createContainer(fpm_image, detach=True, name=self.config.get('base', 'fpm_name'))

        binds = self.getVolumeMapping('fpm')
        # TODO: give the option whether gearman is needed
        self.d.startContainer(fpm_container, binds=binds, links={self.config.get('base', 'gearman_name'): "gearman"})
    ...
    def buildCli(self):
        ...
        cli_container = self.d.createContainer(cli_image, detach=True, name=self.config.get('base', 'cli_name'), entry_point=entry_point)
        binds = self.getVolumeMapping('cli')
        # TODO: give the option whether gearman is needed
        self.d.startContainer(cli_container, binds=binds, links={self.config.get('base', 'gearman_name'): "gearman"})
